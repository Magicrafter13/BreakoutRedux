...
void Ball::Update(std::vector<Brick*> sBricks, Paddle sPaddle) {
	if (speed == 0.0) {
		x = sPaddle.CenterX();
		return;
	}
...
void Ball::Reset(double sX, double sY, double sRadius) {
	x = sX;
	y = sY;
	radius = sRadius;
	do
...
Ball::Ball(double sX, double sY, double sRadius) {
	Reset(sX, sY, sRadius);
}
