...
int main(int argc, char **argv) {
	// TODO: create/open debug text file
	...
	romfsInit();
	csndInit();
	// TODO: set background color to RGB 149, 149, 149, with alpha of 255 (if applicable)
	...
	// TODO: initialize audio (if applicable still)
	...
	// TODO: create paddle and ball objects (create classes for them first)
	...
	// TODO: read settings file if it exists, otherwise write it, also update it if applicable
	...
	while (aptMainLoop()) {
		...
		switch (GameState) {
			case state_title:
				...
				C2D_SceneBegin(top);
				/*
				 * TODO: Write text (only implement the system from before if there is still
				 *       screen tearing)
				 *       Clear consoles (make function to clear them and reset text as well,
				 *       to do the job of CRESET)
			...
			case state_game:
				...
				if (kDown & KEY_SELECT) // TODO: change to A upon release
					CurGame->Shoot();
				if (kHeld & KEY_LEFT)
			...
			case state_betathanks:
				// TODO: just OR the inputs you don't want to accept and invert (~)
			...
			case state_editor:
				...
				for (int y = 0; y < 5; ++y)
					for (int x = 0; x < 10; ++x)
						if (workingLevel[y][x] > 0)
							Brick(x * 40 + 2, y * 20 + 2, 36, 16, workingLevel[y][x] - 1).Draw(); // TODO: this is disgusting
				C3D_FrameEnd(0);
				++frame;
				break;
			case state_exit:
		...
		if (UpdateText) {
			switch (GameState) {
				case state_title:
					...
					// TODO: Create a graphic text class, specifically for the press [button] to start game text to flash every 30 frames it toggles
					break;
				case state_game:
					consoleSelect(&bottomScreen);
					consoleClear();
			...
			std::cout << "                                        "; // TODO: maybe add a randomized message
	...
	// TODO: close each custom level file
